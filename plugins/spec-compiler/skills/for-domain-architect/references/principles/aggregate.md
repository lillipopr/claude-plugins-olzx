...
| **负责维护一致性边界吗？** | 封装业务不变量 | Order 维护订单总额不变量 |
| **拥有独立的生命周期吗？** | 可独立创建、删除 | Order 可独立创建 |
| **是外部访问的唯一入口吗？** | 外部只能通过它访问内部对象 | 通过 Order 访问 OrderItem |
...
**判断结果**：四个问题都答"是" → 实体候选（若是局部 ID 则为子实体，若是全局 ID 则为聚合根候选）
...
| 问题 | 说明 | 示例 |
|------|------|------|
| **只关注属性值吗（不需要 ID 区分身份）？** | 不需要 ID，只关心数值或状态本身 | 颜色 `#FFFFFF`，坐标 `(x,y)` |
| **属性相同即可互换吗（如两个 100 元没区别）？** | 两个对象的属性完全一致时，视为相等 | 两个 `100 CNY` 的金额对象是相等的 |
| **是不可变 (Immutable) 的吗（修改即替换整个对象）？** | 修改意味着替换整个对象，而非修改内部状态 | 修改地址时，创建一个新 `Address` 对象替换旧的 |
| **生命周期依附于他人吗（随宿主同时销毁）？** | 随宿主创建而创建，随宿主销毁而销毁 | 订单被删除，订单内的 `收货地址` 也随之消失 |
